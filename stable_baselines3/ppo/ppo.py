...
class PPO(OnPolicyAlgorithm):
    ...
    def _record_param_norms(self) -> None:
        ...
        spectral = float(sum(spectral_norms) / len(spectral_norms))
        nuclear = float(sum(nuclear_norms) / len(nuclear_norms))
        rank = float(sum(ranks) / len(ranks))

        self.logger.record("param_norms/frobenius", frobenius_norm)
        self.logger.record("param_norms/spectral", spectral)
        self.logger.record("param_norms/nuclear", nuclear)
        self.logger.record("param_norms/rank", rank)
